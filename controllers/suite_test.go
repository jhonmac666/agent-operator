...
var _ = BeforeSuite(func() {
	...
	err = vcv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())
...
func SetupTest(ctx context.Context) *v1.Namespace {
	...
	BeforeEach(func() {
		*ns = v1.Namespace{
			ObjectMeta: metav1.ObjectMeta{Name: VcAgentNamespace},
		}

		err := k8sClient.Create(ctx, ns)
		Expect(err).NotTo(HaveOccurred(), "failed to create \"vcagent\" test namespace")
	})

	AfterEach(func() {
		err := k8sClient.Delete(ctx, ns)
		Expect(err).NotTo(HaveOccurred(), "failed to delete \"vcagent\" test namespace")
	})
