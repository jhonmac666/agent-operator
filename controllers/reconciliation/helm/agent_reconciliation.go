...
const (
	helmRepo       = "https://jhonmac666.github.io/helm-charts"
...
func (h *HelmReconciliation) Delete(_ ctrl.Request, _ *vcV1.VcAgent) error {
	...
	if _, err := uninstallAction.Run(h.crAppName); err != nil {
		...
		if errors.Is(err, driver.ErrReleaseNotFound) {
			h.log.Info("Ignoring error during VC Agent deletion, Helm resources already removed")
	...
	h.log.Info("Release VC Agent (Charts) uninstalled")
	return nil
}

func (h *HelmReconciliation) CreateOrUpdate(_ ctrl.Request, crdInstance *vcV1.VcAgent) error {
	...
	if _, err := histClient.Run(h.crAppName); err == driver.ErrReleaseNotFound {
		h.log.Info("VC Agent Chart Release does not exist. Installing it now.")
		...
		if err != nil {
			h.log.Error(err, "Failure loading or validating VC Agent Helm Chart, cannot proceed installation")
			return errors.Wrap(err, "loading VC Agent Helm Chart failed")
		}

		_, err = installAction.Run(agentChart, yamlMap)
		if err != nil {
			h.log.Error(err, "Failure installing VC Agent Helm Chart, cannot proceed installation")
			return errors.Wrap(err, "installation VC Agent failed")
		}

	} else if err == nil {
		h.log.Info("Found existing VC Agent Chart Release. Upgrade existing installation.")
		...
		h.log.Error(err, "Unexpected error trying to fetch VC Agent Chart install history")
		return err
	}

	h.log.Info("Done installing / upgrading VC Agent Helm Chart")
	return nil
}

func (h *HelmReconciliation) mapCRDToYaml(crdInstance *vcV1.VcAgent) (map[string]interface{}, error) {
