...
func NewAgentChartPostRenderer(h *HelmReconciliation, crdInstance *vcV1.VcAgent) *AgentChartPostRenderer {
...
func (p *AgentChartPostRenderer) Run(in *bytes.Buffer) (*bytes.Buffer, error) {
	...
	if err := resourceList.Visit(func(r *resource.Info, incomingErr error) error {
		...
		if r.ObjectName() == "daemonsets/vcagent" {
		...
		if !(r.ObjectName() == "clusterroles/vcagent" || r.ObjectName() == "clusterrolebindings/vcagent") {
...
func (p *AgentChartPostRenderer) replaceLeaderElectorEnvVar(envList []v1.EnvVar) []v1.EnvVar {
	for i, envVar := range envList {
		if envVar.Name == "VC_AGENT_LEADER_ELECTOR_PORT" {
			envList = append(envList[:i], envList[i+1:]...)
		}
	}
	envList = append(envList, v1.EnvVar{Name: "VC_OPERATOR_MANAGED", Value: "true"})
	return envList
}
