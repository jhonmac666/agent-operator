...
BUNDLE_IMG ?= agent-operator-bundle:$(VERSION)
...
IMG ?= jm666/agent-operator:latest
...
install: manifests kustomize ## Install CRDs into the K8s cluster specified in ~/.kube/config.
	...
	kubectl create ns vcagent || true
	...
	while ! kubectl get secret webhook-server-cert -n vcagent; do echo "...Waiting for Certs to be created" && sleep 2; done
	# Download generated certificates to local, because for WebHooks TLS is mandatory
	kubectl get secret webhook-server-cert -n vcagent --template='{{index .data "ca.crt" | base64decode}}' > testcerts/ca.crt
	kubectl get secret webhook-server-cert -n vcagent --template='{{index .data "tls.crt" | base64decode}}' > testcerts/tls.crt
	kubectl get secret webhook-server-cert -n vcagent --template='{{index .data "tls.key" | base64decode}}' > testcerts/tls.key

uninstall: manifests kustomize ## Uninstall CRDs from the K8s cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/local_development | kubectl delete -f -
	kubectl delete ns vcagent
...
deploy-minikube: manifests kustomize ## Convenience target to push the docker image to a local running Minikube cluster and deploy the Operator there.
	...
	cd config/manager && $(KUSTOMIZE) edit set image jm666/agent-operator=${IMG}
...
operator-sdk: ## Download the Operator SDK binary locally if necessary.
	$(call curl-get-tool,$(OPERATOR_SDK),https://github.com/operator-framework/operator-sdk/releases/download/v1.12.0,operator-sdk_$${OS}_$${ARCH})
